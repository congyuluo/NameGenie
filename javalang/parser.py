...
# -- Top level units --
    ...
    def parse_import_declaration(self):
        ...
        import_position = self.tokens.look().position
        ...
        import_node = tree.Import(path='.'.join(qualified_identifier),
                           static=static,
                           wildcard=import_all)

        import_node._position = import_position
    ...
    def parse_class_or_interface_declaration(self):
        modifier_start_position = self.tokens.look().position
        ...
        if token.value == 'class':
            ...
            type_declaration = self.parse_member_declaration()
        ...
        type_declaration._modifier_position = modifier_start_position
    ...
    def parse_normal_class_declaration(self):
        ...
        name_position = self.tokens.look().position
        ...
        class_declaration = tree.ClassDeclaration(name=name,
                                     type_parameters=type_params,
                                     extends=extends,
                                     implements=implements,
                                     body=body)

        class_declaration._position = name_position
    ...
    def parse_enum_declaration(self):
        ...
        enum_position = self.tokens.look().position
        name = self.parse_identifier()
        ...
        enum_declaration._position = enum_position
    ...
    def parse_normal_interface_declaration(self):
        ...
        interface_declaration._position = name_position
    ...
    def parse_annotation_type_declaration(self):
        ...
        annotation_declaration._position = self.tokens.look().position
...
# -- Types --
    ...
    def parse_reference_type(self):
        ...
        # Added functionality to include position with reference type
        reference_type._position = self.tokens.look().position
        ...
        while True:
            ...
            if self.try_accept('.'):
                ...
                # Added functionality to include position with reference type
                tail.sub_type._position = self.tokens.look().position
                tail = tail.sub_type
            else:
                break
...
# -- Class body --
    ...
    def parse_member_declaration(self):
        ...
        member._modifier_position = modifier_start_position
    ...
    def parse_method_or_field_declaraction(self):
        ...
        name_location = self.tokens.look().position
        member_name = self.parse_identifier()

        member = self.parse_method_or_field_rest(name_location)
        ...
        if isinstance(member, tree.MethodDeclaration):
            ...
            member._position = name_location
        else:
    ...
    @parse_debug
    def parse_method_or_field_rest(self, name_pos):
        token = self.tokens.look()
        
        if self.would_accept('('):
            return self.parse_method_declarator_rest()
        else:
            rest = self.parse_field_declarators_rest(name_pos)
            self.accept(';')
            return rest

    @parse_debug
    def parse_field_declarators_rest(self, name_pos):
        ...
        declarators[0]._position = name_pos

        while self.try_accept(','):
            declarator = self.parse_variable_declarator()
            declarators.append(declarator)

        field_declaration = tree.FieldDeclaration(declarators=declarators)
        field_declaration._position = self.tokens.look().position
    ...
    def parse_method_declarator_rest(self):
        ...
        method_declaration = tree.MethodDeclaration(parameters=formal_parameters,
                                     throws=throws,
                                     body=body,
    ...
    def parse_constructor_declarator_rest(self):
        ...
        constructor_declaration = tree.ConstructorDeclaration(parameters=formal_parameters,
                                           throws=throws,
                                           body=body)
    ...
    def parse_generic_method_or_constructor_declaration(self):
        ...
        if self.would_accept(Identifier, '('):
            ...
            method._position = constructor_position
        elif self.try_accept('void'):
            method_position = self.tokens.look().position
            method_name = self.parse_identifier()
            method = self.parse_void_method_declarator_rest()
            method.name = method_name
            method._position = method_position

        else:
            method_return_type = self.parse_type()
            method_name_position = self.tokens.look().position
            method_name = self.parse_identifier()
            ...
            method.name = method_name
            method._position = method_name_position
...
# -- Interface body --
    ...
    def parse_interface_body_declaration(self):
        ...
        modifier_position = self.tokens.look().position
        modifiers, annotations, javadoc = self.parse_modifiers()
        ...
        declaration._modifier_position = modifier_position
    ...
    def parse_interface_member_declaration(self):
        ...
        if self.would_accept('class'):
            ...
            method_name_location = self.tokens.look().position
            method_name = self.parse_identifier()
            declaration = self.parse_void_interface_method_declarator_rest()
            declaration.name = method_name
            declaration._position = method_name_location
        else:
    ...
    def parse_interface_method_or_field_declaration(self):
        ...
        if isinstance(member, tree.MethodDeclaration):
            ...
            member._position = name_position
        else:
    ...
    def parse_constant_declarator(self):
        ...
        result = tree.VariableDeclarator(name=name,
                                       dimensions=additional_dimension,
                                       initializer=initializer,)
        result._position = name_position
        return
    ...
    def parse_interface_method_declarator_rest(self):
        ...
        method_declaration = tree.MethodDeclaration(parameters=parameters,
                                      throws=throws,
                                      body=body,
...
# -- Parameters and variables --
    ...
    def parse_formal_parameters(self):
        ...
        while True:
            ...
            identifier_position = self.tokens.look().position
            ...
            parameter._position = identifier_position
    ...
    def parse_variable_declarator(self):
        ...
        result = tree.VariableDeclarator(name=identifier,
                                       dimensions=array_dimension,
                                       initializer=initializer)
        result._position = name_location
...
# -- Blocks and statements --
    ...
    def parse_local_variable_declaration_statement(self):
        ...
        # After parsing type, capture position
        var_position = self.tokens.look().position
...
# -- Try / catch --
    ...
    def parse_catch_clause(self):
        ...
        catch_parameter._position = self.tokens.look().position
        ...
        catch_parameter._position = name_position
    ...
    def parse_resource(self):
        ...
        try_resource._position = self.tokens.look().position
...
# -- Primary expressions --
    ...
    def parse_primary(self):
        ...
        if isinstance(token, Literal):
            ...
            suffix_type_position = self.tokens.look().position
            ...
            if isinstance(identifier_suffix, (tree.MemberReference, tree.MethodInvocation)):
                ...
                identifier_suffix._position = suffix_type_position
                ...
                identifier_suffix.type._position = suffix_type_position
    ...
    def parse_explicit_generic_invocation_suffix(self):
        ...
        if self.try_accept('super'):
            ...
            identifier = self.parse_identifier()
            arguments = self.parse_arguments()
            result = tree.MethodInvocation(member=identifier,
                                         arguments=arguments)
            result._position = identifier_position
            return result
...
# -- Creators --
    ...
    def parse_created_name(self):
        ...
        # Added functionality to include position with reference type
        created_name._position = self.tokens.look().position
    ...
    def parse_selector(self):
        if self.try_accept('['):
            ...
            if isinstance(token, Identifier):
                ...
                if self.would_accept('('):
                    arguments = self.parse_arguments()
                    result = tree.MethodInvocation(member=identifier, arguments=arguments)
                    result._position = identifier_position
                    return result
                else:
                    result = tree.MemberReference(member=identifier)
                    result._position = identifier_position
                    return result
            elif self.would_accept('super', '::'):
                self.accept('super')
...
# -- Enum and annotation body --
    ...
    def parse_enum_constant(self):
        ...
        enum_constant_declaration = tree.EnumConstantDeclaration(annotations=annotations,
                                            name=constant_name,
                                            arguments=arguments,
                                            body=body,
                                            documentation=javadoc)
        enum_constant_declaration._position = next_token.position
    ...
    def parse_annotation_type_element_declaration(self):
        ...
        if self.would_accept('class'):
            ...
            if isinstance(declaration, tree.AnnotationMethod):
                ...
                declaration._position = name_position
                declaration.return_type = attribute_type
            else:
                declaration.declarators[0].name = attribute_name
                declaration.declarators[0]._position = name_position
    ...
    def parse_annotation_method_or_constant_rest(self):
        if self.try_accept('('):
            ...
            # Added functionality to include position with annotation method
            annotation_method = tree.AnnotationMethod(dimensions=array_dimension,
                                         default=default)
            annotation_method._position = self.tokens.look().position
